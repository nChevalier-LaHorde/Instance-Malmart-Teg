...
CH3TmxLayerChunk::CH3TmxLayerChunk(const tmx::TileLayer& layer, std::vector<const tmx::Tileset*> tilesets, const sf::Vector2f& position, const sf::Vector2f& tileCount, const sf::Vector2u& tileSize, std::size_t rowSize, H3TextureResource& tr, const std::map<std::uint32_t, tmx::Tileset::Tile>& animTiles)
    : m_animTiles(animTiles)
{
    ...
    for (const auto& ts : tilesets)
    {
        ...
        m_chunkArrays.emplace_back(std::make_unique<CH3TmxLayerChunkArray>(*tr.find(ts->getImagePath())->second, *ts));
    }
...
void CH3TmxLayerChunk::generateTiles(bool registerAnimation)
{
...
tmx::TileLayer::Tile CH3TmxLayerChunk::getTile(int x, int y) const
{
    return m_chunkTileIDs[calcIndexFrom(x, y)];
}
void CH3TmxLayerChunk::setTile(int x, int y, tmx::TileLayer::Tile tile, bool refresh)
{
    m_chunkTileIDs[calcIndexFrom(x, y)] = tile;
    maybeRegenerate(refresh);
}

sf::Color CH3TmxLayerChunk::getColor(int x, int y) const
{
    return m_chunkColors[calcIndexFrom(x, y)];
}

void CH3TmxLayerChunk::setColor(int x, int y, sf::Color color, bool refresh)
{
    m_chunkColors[calcIndexFrom(x, y)] = color;
    maybeRegenerate(refresh);
}

void CH3TmxLayerChunk::maybeRegenerate(bool refresh)
{
    if (refresh)
    {
        for (const auto& ca : m_chunkArrays)
...
int CH3TmxLayerChunk::calcIndexFrom(int x, int y) const
{
    return x + y * chunkTileCount.x;
}

bool CH3TmxLayerChunk::empty() const { return m_chunkArrays.empty(); }

void CH3TmxLayerChunk::flipY(sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2, sf::Vector2f* v3)
{
    //Flip Y
    sf::Vector2f tmp = *v0;
    v0->y = v2->y;
...
void CH3TmxLayerChunk::flipX(sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2, sf::Vector2f* v3)
{
    //Flip X
    sf::Vector2f tmp = *v0;
    v0->x = v1->x;
...
void CH3TmxLayerChunk::flipD(sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2, sf::Vector2f* v3)
{
    //Diagonal flip
    sf::Vector2f tmp = *v1;
    v1->x = v3->x;
...
void CH3TmxLayerChunk::doFlips(std::uint8_t bits, sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2, sf::Vector2f* v3)
{
    //0000 = no change
...
void CH3TmxLayerChunk::draw(sf::RenderTarget& rt, sf::RenderStates states) const
{
...
CH3TmxLayer::CH3TmxLayer(const tmx::Map& map, size_t idx)
{
...
const sf::FloatRect& CH3TmxLayer::getGlobalBounds() const { return m_globalBounds; }

void CH3TmxLayer::setTile(int tileX, int tileY, tmx::TileLayer::Tile tile, bool refresh)
{
...
tmx::TileLayer::Tile CH3TmxLayer::getTile(int tileX, int tileY)
{
    sf::Vector2u chunkLocale;
    const auto& selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
...
void CH3TmxLayer::setColor(int tileX, int tileY, sf::Color color, bool refresh)
{
    sf::Vector2u chunkLocale;
    const auto& selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
...
sf::Color CH3TmxLayer::getColor(int tileX, int tileY)
{
    sf::Vector2u chunkLocale;
    const auto& selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
...
void CH3TmxLayer::update(sf::Time elapsed)
{
    for (auto& c : m_visibleChunks)
    {
        for (SH3TmxAnimationState& as : c->getActiveAnimations())
        {
...
CH3TmxLayerChunk::Ptr& CH3TmxLayer::getChunkAndTransform(int x, int y, sf::Vector2u& chunkRelative)
{
...
void CH3TmxLayer::createChunks(const tmx::Map& map, const tmx::TileLayer& layer)
{
    ...
    for (auto y = 0u; y < m_chunkCount.y; ++y)
    {
        ...
        for (auto x = 0u; x < m_chunkCount.x; ++x)
        {
            ...
            m_chunks.emplace_back(std::make_unique<CH3TmxLayerChunk>(layer, usedTileSets,
                sf::Vector2f(x * m_chunkSize.x, y * m_chunkSize.y), tileCount, tileSize, map.getTileCount().x, m_textureResource, map.getAnimatedTiles()));
        }
    }
}

void CH3TmxLayer::updateVisibility(const sf::View& view) const
{
    ...
    std::vector<CH3TmxLayerChunk*> visible;
...
void CH3TmxLayer::draw(sf::RenderTarget& rt, sf::RenderStates states) const
{
...
CH3TmxLayerChunkArray::CH3TmxLayerChunkArray(const sf::Texture& t, const tmx::Tileset& ts)
    : m_texture(t)
{
...
void CH3TmxLayerChunkArray::reset()
{
    m_vertices.clear();
}

void CH3TmxLayerChunkArray::addTile(const H3TmxLayerChunkTile& tile)
{
...
sf::Vector2u CH3TmxLayerChunkArray::getTextureSize() const { return m_texture.getSize(); }

void CH3TmxLayerChunkArray::draw(sf::RenderTarget& rt, sf::RenderStates states) const
{
